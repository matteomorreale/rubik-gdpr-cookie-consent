...
class Manus_GDPR_Frontend {
    ...
    private $public_handle = 'matteomorreale-gdpr';
    ...
    public function enqueue_scripts() {
        ...
        wp_localize_script( $this->public_handle, 'matteomorreale_gdpr_ajax', array(
            'ajax_url' => admin_url( 'admin-ajax.php' ),
            'nonce'    => wp_create_nonce( 'matteomorreale_gdpr_consent_action' ),
            'settings' => array(
    ...
    public function display_cookie_banner() {
        ...
        $privacy_page_id = 0;
        if ( isset( $options['privacy_policy_page_id'] ) ) {
            $privacy_page_id = absint( $options['privacy_policy_page_id'] );
        } elseif ( isset( $options['privacy_page_id'] ) ) {
            $privacy_page_id = absint( $options['privacy_page_id'] );
        }

        $cookie_policy_page_id = isset( $options['cookie_policy_page_id'] ) ? absint( $options['cookie_policy_page_id'] ) : 0;
        ...
        if ( $cookie_policy_page_id ) {
            $cookie_policy_link = get_permalink( $cookie_policy_page_id );
        }
    ...
    public function handle_consent_action() {
        $nonce = '';
        if ( isset( $_POST['matteomorreale_gdpr_consent_nonce'] ) ) {
            $nonce = $_POST['matteomorreale_gdpr_consent_nonce'];
        } elseif ( isset( $_POST['manus_gdpr_consent_nonce'] ) ) {
            $nonce = $_POST['manus_gdpr_consent_nonce'];
        }

        if (
            empty( $nonce ) ||
            ( ! wp_verify_nonce( $nonce, 'matteomorreale_gdpr_consent_action' ) && ! wp_verify_nonce( $nonce, 'manus_gdpr_consent_action' ) )
        ) {
    ...
    public function add_cookie_blocking_script() {
        ...
        (function() {
            ...
            window.matteomorrealeGDPRCookieBlocker = {
    ...
    public function add_tcf_v2_script() {
        ...
        (function() {
            ...
            document.addEventListener('matteomorreale-gdpr-consent-updated', updateTCFStatus);
    ...
    private function add_custom_css() {
        ...
        if ( ! empty( $custom_css ) ) {
            $custom_css = str_replace(
                array(
                    '.manus-gdpr-button.primary',
                    '.manus-gdpr-button.secondary',
                    '.manus-gdpr-button',
                    '.manus-gdpr-accept',
                    '.manus-gdpr-reject',
                    '.manus-gdpr-preferences',
                    '.manus-gdpr-banner.position-top',
                    '.manus-gdpr-banner.position-bottom',
                    '.manus-gdpr-banner.position-center',
                    '.manus-gdpr-banner',
                    '.manus-gdpr-message',
                    '.manus-gdpr-buttons',
                    '.manus-gdpr-modal-overlay',
                    '.manus-gdpr-modal',
                    '.manus-gdpr-modal-header',
                    '.manus-gdpr-modal-body',
                    '.manus-gdpr-modal-footer',
                    '.manus-gdpr-close',
                ...
                array(
                    ...
                    '#matteomorreale-gdpr-cookie-banner',
                    '.matteomorreale-gdpr-banner-text',
            ...
            wp_add_inline_style( $this->public_handle, $this->sanitize_css( $custom_css ) );
        }
    }
