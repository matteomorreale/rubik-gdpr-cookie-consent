...
<div class="wrap">
    <h1><?php _e( 'Cookie Scanner', 'm2-gdpr' ); ?></h1>

    <div id="cookie-scanner-container">
        
        <!-- Scanner Section -->
        <div class="postbox">
            <h2 class="hndle"><?php _e( 'Scansiona una pagina', 'm2-gdpr' ); ?></h2>
            <div class="inside">
                <p><?php _e( 'Inserisci l\'URL di una pagina del tuo sito per analizzare i cookie che vengono caricati. Lo scanner aprirà la pagina e rileverà automaticamente tutti i cookie presenti.', 'm2-gdpr' ); ?></p>
                
                <div class="scanner-form">
                    <div class="scanner-input-group">
                        <label for="scan-url"><?php _e( 'URL da scansionare:', 'm2-gdpr' ); ?></label>
                    ...
                    <div class="scanner-options">
                        <label>
                            ...
                            <?php _e( 'Pulisci cookie esistenti prima della scansione', 'm2-gdpr' ); ?>
                        </label>
                        <label>
                            <input type="checkbox" id="wait-for-load" checked>
                            <?php _e( 'Attendi il caricamento completo della pagina', 'm2-gdpr' ); ?>
                        </label>
                        <label>
                            <input type="checkbox" id="deep-scan">
                            <?php _e( 'Scansione approfondita (più lenta ma più accurata)', 'm2-gdpr' ); ?>
                        </label>
                    </div>
                </div>
                ...
                <div id="scanner-progress" class="scanner-progress" style="display: none;">
                    ...
                    <p class="progress-text"><?php _e( 'Scansione in corso...', 'm2-gdpr' ); ?></p>
                </div>
        ...
        <div class="postbox">
            <h2 class="hndle"><?php _e( 'Risultati delle scansioni', 'm2-gdpr' ); ?></h2>
            <div class="inside">
                <div class="scan-results-tabs">
                    <nav class="nav-tab-wrapper">
                        <a href="#" class="nav-tab nav-tab-active" data-tab="recent"><?php _e( 'Scansioni recenti', 'm2-gdpr' ); ?></a>
                        <a href="#" class="nav-tab" data-tab="categories"><?php _e( 'Per categoria', 'm2-gdpr' ); ?></a>
                        <a href="#" class="nav-tab" data-tab="blocked"><?php _e( 'Cookie bloccati', 'm2-gdpr' ); ?></a>
                    </nav>
                    ...
                    <div id="tab-recent" class="tab-content active">
                        <table class="wp-list-table widefat fixed striped">
                            <thead>
                                <tr>
                                    <th><?php _e( 'URL', 'm2-gdpr' ); ?></th>
                                    <th><?php _e( 'Data scansione', 'm2-gdpr' ); ?></th>
                                    <th><?php _e( 'Cookie trovati', 'm2-gdpr' ); ?></th>
                                    <th><?php _e( 'Categorie', 'm2-gdpr' ); ?></th>
                                    <th><?php _e( 'Azioni', 'm2-gdpr' ); ?></th>
                                </tr>
                            </thead>
                            <tbody>
                            ...
                            <?php if ( ! empty( $cookie_categories ) ): ?>
                                ...
                                <div class="category-card category-<?php echo esc_attr( $category_key ); ?>">
                                    ...
                                    <div class="category-patterns">
                                        <strong><?php _e( 'Pattern riconosciuti:', 'm2-gdpr' ); ?></strong>
                                        <span class="patterns"><?php echo esc_html( implode( ', ', $category_data['patterns'] ) ); ?></span>
                                    </div>
                                </div>
                                <?php endforeach; ?>
                            <?php else: ?>
                                <p><?php _e( 'Nessuna categoria di cookie definita.', 'm2-gdpr' ); ?></p>
                            <?php endif; ?>
                        </div>
                    </div>
                    ...
                    <div id="tab-blocked" class="tab-content">
                        <div id="blocked-cookies-list">
                            <p><?php _e( 'Seleziona una scansione per vedere i cookie bloccati.', 'm2-gdpr' ); ?></p>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div id="scan-details-modal" class="scan-modal" style="display: none;">
        <div class="scan-modal-content">
            <div class="scan-modal-header">
                <h2><?php _e( 'Dettagli della scansione', 'm2-gdpr' ); ?></h2>
            ...
            <div class="scan-modal-footer">
                <button type="button" class="button button-primary" id="save-cookie-settings">
                    <?php _e( 'Salva impostazioni', 'm2-gdpr' ); ?>
                </button>
                <button type="button" class="button scan-modal-close">
                    <?php _e( 'Chiudi', 'm2-gdpr' ); ?>
                </button>
            </div>
        </div>
    </div>
...
jQuery(document).ready(function($) {
    ...
    const scanner = {
        ...
        startScan: function() {
            ...
            if (!url) {
                alert('<?php _e( "Inserisci un URL valido", "m2-gdpr" ); ?>');
                return;
            }
        ...
        quickTest: function() {
            this.showProgress();
            $('.progress-text').text('<?php _e( "Esecuzione test rapido...", "m2-gdpr" ); ?>');
            ...
            setTimeout(() => {
                ...
                $('.progress-text').text('<?php _e( "Test completato!", "m2-gdpr" ); ?>');
        ...
        performScan: function(url) {
            ...
            iframe.onload = () => {
                setTimeout(() => {
                    ...
                    $('.progress-fill').css('width', '100%');
                    $('.progress-text').text('<?php _e( "Analisi completata!", "m2-gdpr" ); ?>');
        ...
        performDeepScan: function(url) {
            ...
            ManusGDPRCookieScanner.performDeepScan(url, {
                ...
                alert('<?php _e( "Errore durante la scansione approfondita", "m2-gdpr" ); ?>');
                this.hideProgress();
            });
        },
        ...
        extractCookies: function(iframe, url) {
            try {
                ...
                if (allCookies.length === 0) {
                    alert('<?php _e( "Nessun cookie rilevato su questa pagina", "m2-gdpr" ); ?>');
                    this.hideProgress();
                    return;
                }
                ...
                alert('<?php _e( "Errore durante l\'estrazione dei cookie", "m2-gdpr" ); ?>');
                this.hideProgress();
            }
        },
        ...
        saveCookiesToServer: function(url, cookies) {
            $.ajax({
                ...
                success: (response) => {
                    if (response.success) {
                        alert('<?php _e( "Scansione completata con successo!", "m2-gdpr" ); ?>');
                        location.reload(); // Refresh to show new results
                    } else {
                        alert('<?php _e( "Errore durante il salvataggio: ", "m2-gdpr" ); ?>' + response.data);
                    }
                    this.hideProgress();
                },
                error: () => {
                    alert('<?php _e( "Errore di comunicazione con il server", "m2-gdpr" ); ?>');
                    this.hideProgress();
                }
            });
        },
        ...
        loadScanDetails: function(scanId) {
            ...
            $('#scan-details-content').html('<p><?php _e( "Caricamento...", "m2-gdpr" ); ?></p>');
            ...
            $.ajax({
                ...
                success: (response) => {
                    if (response.success) {
                        ...
                        $('#scan-details-content').html('<p><?php _e( "Errore nel caricamento dei dettagli", "m2-gdpr" ); ?></p>');
                    }
                }
            });
        },
        ...
        renderScanDetails: function(scanResults, scanId) {
            ...
            let html = `<div class="scan-details">
                <h3><?php _e( "URL:", "m2-gdpr" ); ?> ${scan.url}</h3>
                <p><?php _e( "Data scansione:", "m2-gdpr" ); ?> ${scan.scan_date}</p>
                <p><?php _e( "Cookie trovati:", "m2-gdpr" ); ?> ${scan.cookies_count}</p>
                <table class="wp-list-table widefat fixed striped">
                    <thead>
                        <tr>
                            <th><?php _e( "Nome", "m2-gdpr" ); ?></th>
                            <th><?php _e( "Categoria", "m2-gdpr" ); ?></th>
                            <th><?php _e( "Dominio", "m2-gdpr" ); ?></th>
                            <th><?php _e( "Scadenza", "m2-gdpr" ); ?></th>
                            <th><?php _e( "Permetti", "m2-gdpr" ); ?></th>
                        </tr>
                    </thead>
                    <tbody>`;
            ...
            scan.cookies.forEach(cookie => {
                ...
                html += `<tr>
                    ...
                    <td>${cookie.domain}</td>
                    <td>${cookie.expires || '<?php _e( "Sessione", "m2-gdpr" ); ?>'}</td>
        ...
        saveCookieSettings: function() {
            ...
            $.ajax({
                ...
                success: (response) => {
                    if (response.success) {
                        alert('<?php _e( "Impostazioni salvate con successo!", "m2-gdpr" ); ?>');
                        this.closeModal();
                    } else {
                        alert('<?php _e( "Errore nel salvare le impostazioni", "m2-gdpr" ); ?>');
                    }
                }
            });
        },
        ...
        deleteScan: function(e) {
            if (!confirm('<?php _e( "Sei sicuro di voler eliminare questa scansione?", "m2-gdpr" ); ?>')) {
                return;
            }
            ...
            $.ajax({
                ...
                success: (response) => {
                    if (response.success) {
                        alert('<?php _e( "Scansione eliminata con successo!", "m2-gdpr" ); ?>');
                        location.reload();
                    } else {
                        alert('<?php _e( "Errore nell\'eliminare la scansione", "m2-gdpr" ); ?>');
                    }
                }
            });
        },
